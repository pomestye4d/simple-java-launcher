...
import com.vga.sjl.external.org.snakeyaml.engine.v2.exceptions.SjlExtReaderException;
 ...
 */
public final class SjlExtStreamReader {
   ...
   */
  public SjlExtStreamReader(Reader reader, SjlExtLoadSettings loadSettings) {
    this(loadSettings, reader);
  }

  public SjlExtStreamReader(SjlExtLoadSettings loadSettings, Reader reader) {
   ...
   * @deprecated use the other constructor with LoadSettings first
   */
  public SjlExtStreamReader(String stream, SjlExtLoadSettings loadSettings) {
    this(loadSettings, new StringReader(stream));
  }

  public SjlExtStreamReader(SjlExtLoadSettings loadSettings, String stream) {
    this(loadSettings, new StringReader(stream));
  }
  ...
  /**
   * Generate {@link SjlExtMark} if it is configured
   *
   * @return {@link SjlExtMark} of the current position or empty {@link Optional} otherwise
   */
  public Optional<SjlExtMark> getMark() {
    if (useMarks) {
      return Optional.of(
          new SjlExtMark(name, this.index, this.line, this.column, this.codePointsWindow,
              this.pointer));
    } else {
  ...
  public void forward(int length) {
    for (int i = 0; i < length && ensureEnoughData(); i++) {
      ...
      this.index++;
      if (SjlExtCharConstants.LINEBR.has(c)
    ...
    try {
      ...
      if (read > 0) {
        ...
        if (nonPrintable.isPresent()) {
          throw new SjlExtReaderException(name, cpIndex - 1, nonPrintable.get(),
      ...
      throw new SjlExtYamlEngineException(ioe);
    }
  }
