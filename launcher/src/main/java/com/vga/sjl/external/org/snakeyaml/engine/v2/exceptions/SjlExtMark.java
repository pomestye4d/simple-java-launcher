 ...
 */
public final class SjlExtMark implements Serializable {
   ...
   */
  public SjlExtMark(String name, int index, int line, int column, int[] buffer, int pointer) {
    super();
    this.name = name;
    this.index = index;
    this.line = line;
   ...
   * @param pointer - the position of the mark from the beginning of the stream
   */
  public SjlExtMark(String name, int index, int line, int column, char[] str, int pointer) {
    this(name, index, line, column, toCodePoints(str), pointer);
  }

  private boolean isLineBreak(int c) {
    return SjlExtCharConstants.NULL_OR_LINEBR.has(c);
  }
