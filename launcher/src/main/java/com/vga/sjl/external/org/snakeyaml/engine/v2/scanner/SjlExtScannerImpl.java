...
import com.vga.sjl.external.org.snakeyaml.engine.v2.exceptions.SjlExtScannerException;
import com.vga.sjl.external.org.snakeyaml.engine.v2.exceptions.SjlExtYamlEngineException;
import com.vga.sjl.external.org.snakeyaml.engine.v2.scanner.SjlExtScannerImpl.Chomping.Indicator;
...
import static com.vga.sjl.external.org.snakeyaml.engine.v2.common.SjlExtCharConstants.ESCAPE_CODES;
import static com.vga.sjl.external.org.snakeyaml.engine.v2.common.SjlExtCharConstants.ESCAPE_REPLACEMENTS;
 ...
 * </pre>
 */
public final class SjlExtScannerImpl implements SjlExtScanner {
  ...
  private final SjlExtStreamReader reader;
  ...
  private final List<SjlExtToken> tokens;

  // The last added token
  private SjlExtToken lastToken;
   ...
   */
  private final Map<Integer, SjlExtSimpleKey> possibleSimpleKeys;
   ...
   */
  public SjlExtScannerImpl(SjlExtStreamReader reader, SjlExtLoadSettings settings) {
    this(settings, reader);
  }

  public SjlExtScannerImpl(SjlExtLoadSettings settings, SjlExtStreamReader reader) {
    this.reader = reader;
   ...
   */
  public SjlExtScannerImpl(SjlExtStreamReader reader) {
    this(SjlExtLoadSettings.builder().build(), reader);
  }

  /**
   * Check whether the next token is one of the given types.
   */
  public boolean checkToken(SjlExtToken.ID... choices) {
    ...
    if (!this.tokens.isEmpty()) {
      ...
      SjlExtToken firstToken = this.tokens.get(0);
      SjlExtToken.ID first = firstToken.getTokenId();
  ...
  private void addToken(SjlExtToken token) {
    lastToken = token;
    this.tokens.add(token);
  }

  private void addToken(int index, SjlExtToken token) {
  ...
  private void addAllTokens(List<SjlExtToken> tokens) {
  ...
  private void fetchMoreTokens() {
    ...
    String chRepresentation = SjlExtCharConstants.escapeChar(String.valueOf(Character.toChars(c)));
    if (c == '\t') {
    ...
    throw new SjlExtScannerException("while scanning for the next token", Optional.empty(), text,
        reader.getMark());
  }
  ...
  private void stalePossibleSimpleKeys() {
    if (!this.possibleSimpleKeys.isEmpty()) {
      for (Iterator<SjlExtSimpleKey> iterator = this.possibleSimpleKeys.values().iterator();
           iterator.hasNext(); ) {
        SjlExtSimpleKey key = iterator.next();
        if ((key.getLine() != reader.getLine())
          ...
          if (key.isRequired()) {
            ...
            // condition.
            throw new SjlExtScannerException("while scanning a simple key", key.getMark(),
  ...
  private void savePossibleSimpleKey() {
    ...
    if (this.allowSimpleKey) {
      ...
      SjlExtSimpleKey key = new SjlExtSimpleKey(tokenNumber, required, reader.getIndex(),
  ...
  private void removePossibleSimpleKey() {
    SjlExtSimpleKey key = possibleSimpleKeys.remove(flowLevel);
  ...
  private void unwindIndent(int col) {
    ...
    while (this.indent > col) {
      Optional<SjlExtMark> mark = reader.getMark();
      this.indent = this.indents.pop();
      addToken(new SjlExtBlockEndToken(mark, mark));
    }
  }
  ...
  private void fetchStreamStart() {
    ...
    // Add STREAM-START.
    SjlExtToken token = new SjlExtStreamStartToken(mark, mark);
    addToken(token);
  }
  ...
  private void fetchStreamEnd() {
    ...
    // Add STREAM-END.
    SjlExtToken token = new SjlExtStreamEndToken(mark, mark);
    addToken(token);
  ...
  private void fetchDirective() {
    ...
    List<SjlExtToken> tok = scanDirective();
    addAllTokens(tok);
  }
  ...
  private void fetchDocumentIndicator(boolean isDocumentStart) {
    ...
    Optional<SjlExtMark> startMark = reader.getMark();
    reader.forward(3);
    Optional<SjlExtMark> endMark = reader.getMark();
    SjlExtToken token;
    if (isDocumentStart) {
      token = new SjlExtDocumentStartToken(startMark, endMark);
    } else {
      token = new SjlExtDocumentEndToken(startMark, endMark);
    }
    addToken(token);
  }
  ...
  private void fetchFlowCollectionStart(boolean isMappingStart) {
    ...
    SjlExtToken token;
    if (isMappingStart) {
      token = new SjlExtFlowMappingStartToken(startMark, endMark);
    } else {
      token = new SjlExtFlowSequenceStartToken(startMark, endMark);
    }
    addToken(token);
  }
  ...
  private void fetchFlowCollectionEnd(boolean isMappingEnd) {
    ...
    SjlExtToken token;
    if (isMappingEnd) {
      token = new SjlExtFlowMappingEndToken(startMark, endMark);
    } else {
      token = new SjlExtFlowSequenceEndToken(startMark, endMark);
    }
    addToken(token);
  }
  ...
  private void fetchFlowEntry() {
    ...
    reader.forward();
    Optional<SjlExtMark> endMark = reader.getMark();
    SjlExtToken token = new SjlExtFlowEntryToken(startMark, endMark);
    addToken(token);
  }
  ...
  private void fetchBlockEntry() {
    ...
    if (isBlockContext()) {
      ...
      if (!this.allowSimpleKey) {
        throw new SjlExtScannerException("", Optional.empty(), "sequence entries are not allowed here",
            reader.getMark());
      }
      ...
      if (addIndent(this.reader.getColumn())) {
        Optional<SjlExtMark> mark = reader.getMark();
        addToken(new SjlExtBlockSequenceStartToken(mark, mark));
      }
    } else {
    ...
    reader.forward();
    Optional<SjlExtMark> endMark = reader.getMark();
    SjlExtToken token = new SjlExtBlockEntryToken(startMark, endMark);
    addToken(token);
  }
  ...
  private void fetchKey() {
    ...
    if (isBlockContext()) {
      ...
      if (!this.allowSimpleKey) {
        throw new SjlExtScannerException("mapping keys are not allowed here",
            reader.getMark());
      }
      // We may need to add BLOCK-MAPPING-START.
      if (addIndent(this.reader.getColumn())) {
        Optional<SjlExtMark> mark = reader.getMark();
        addToken(new SjlExtBlockMappingStartToken(mark, mark));
      }
    }
    ...
    reader.forward();
    Optional<SjlExtMark> endMark = reader.getMark();
    SjlExtToken token = new SjlExtKeyToken(startMark, endMark);
    addToken(token);
  }
  ...
  private void fetchValue() {
    ...
    SjlExtSimpleKey key = this.possibleSimpleKeys.remove(this.flowLevel);
    if (key != null) {
      // Add KEY.
      addToken(key.getTokenNumber() - this.tokensTaken, new SjlExtKeyToken(key.getMark(),
          key.getMark()));
      ...
      if (isBlockContext() && addIndent(key.getColumn())) {
        addToken(key.getTokenNumber() - this.tokensTaken,
            new SjlExtBlockMappingStartToken(key.getMark(), key.getMark()));
      }
      ...
      if (isBlockContext()) {
        ...
        // start a simple key.
        if (!this.allowSimpleKey) {
          throw new SjlExtScannerException("mapping values are not allowed here", reader.getMark());
        }
      }
    ...
    reader.forward();
    Optional<SjlExtMark> endMark = reader.getMark();
    SjlExtToken token = new SjlExtValueToken(startMark, endMark);
    addToken(token);
  }
  ...
  private void fetchAlias() {
    ...
    // Scan and add ALIAS.
    SjlExtToken tok = scanAnchor(false);
    addToken(tok);
  }
  ...
  private void fetchAnchor() {
    ...
    // Scan and add ANCHOR.
    SjlExtToken tok = scanAnchor(true);
    addToken(tok);
  }
  ...
  private void fetchLiteral() {
    fetchBlockScalar(SjlExtScalarStyle.LITERAL);
  }
  ...
  private void fetchFolded() {
    fetchBlockScalar(SjlExtScalarStyle.FOLDED);
  }
   ...
   * @param style
   */
  private void fetchBlockScalar(SjlExtScalarStyle style) {
    ...
    // Scan and add SCALAR.
    List<SjlExtToken> tok = scanBlockScalar(style);
    addAllTokens(tok);
  }
  ...
  private void fetchSingle() {
    fetchFlowScalar(SjlExtScalarStyle.SINGLE_QUOTED);
  }
  ...
  private void fetchDouble() {
    fetchFlowScalar(SjlExtScalarStyle.DOUBLE_QUOTED);
  }
   ...
   * @param style
   */
  private void fetchFlowScalar(SjlExtScalarStyle style) {
    ...
    // Scan and add SCALAR.
    SjlExtToken tok = scanFlowScalar(style);
    addToken(tok);
  }
  ...
  private boolean checkDocumentStart() {
    ...
    if (reader.getColumn() == 0) {
      return "---".equals(reader.prefix(3)) && SjlExtCharConstants.NULL_BL_T_LINEBR.has(reader.peek(3));
    }
    return false;
  }
  ...
  private boolean checkBlockEntry() {
    // BLOCK-ENTRY: '-' (' '|'\n')
    return SjlExtCharConstants.NULL_BL_T_LINEBR.has(reader.peek(1));
  }
  ...
  private boolean checkPlain() {
    ...
    boolean notForbidden = SjlExtCharConstants.NULL_BL_T_LINEBR.hasNo(c, "-?:,[]{}#&*!|>'\"%@`");
    if (notForbidden) {
      ...
      if (isBlockContext()) {
        ...
        return SjlExtCharConstants.NULL_BL_T_LINEBR.hasNo(reader.peek(1)) && "-?:".indexOf(c) != -1;
      } else {
        // It may also start with  '-', '?' if it is followed by a non-space character
  ...
  private void scanToNextToken() {
    ...
    while (!found) {
      ...
      if (reader.peek() == '#') {
        commentSeen = true;
        SjlExtCommentType type;
        if (columnBeforeComment != 0 && !(lastToken != null
            && lastToken.getTokenId() == SjlExtToken.ID.BlockEntry)) {
          type = SjlExtCommentType.IN_LINE;
          inlineStartColumn = reader.getColumn();
        } else if (inlineStartColumn == reader.getColumn()) {
          type = SjlExtCommentType.IN_LINE;
        } else {
          inlineStartColumn = -1;
          type = SjlExtCommentType.BLOCK;
        }
        SjlExtCommentToken token = scanComment(type);
      ...
      if (breaksOpt.isPresent()) { // found a line-break
        if (settings.getParseComments() && !commentSeen) {
          if (columnBeforeComment == 0) {
            addToken(new SjlExtCommentToken(SjlExtCommentType.BLANK_LINE, breaksOpt.get(), startMark,
                reader.getMark()));
          }
        }
        if (isBlockContext()) {
  ...
  private SjlExtCommentToken scanComment(SjlExtCommentType type) {
    // See the specification for details.
    Optional<SjlExtMark> startMark = reader.getMark();
    reader.forward();
    int length = 0;
    while (SjlExtCharConstants.NULL_OR_LINEBR.hasNo(reader.peek(length))) {
      length++;
    }
    String value = reader.prefixForward(length);
    Optional<SjlExtMark> endMark = reader.getMark();
    return new SjlExtCommentToken(type, value, startMark, endMark);
  }

  @SuppressWarnings({"unchecked", "rawtypes"})
  private List<SjlExtToken> scanDirective() {
    // See the specification for details.
    Optional<SjlExtMark> startMark = reader.getMark();
    ...
    Optional<List<?>> value;
    if (SjlExtDirectiveToken.YAML_DIRECTIVE.equals(name)) {
      value = Optional.of(scanYamlDirectiveValue(startMark));
      endMark = reader.getMark();
    } else if (SjlExtDirectiveToken.TAG_DIRECTIVE.equals(name)) {
      value = Optional.of(scanTagDirectiveValue(startMark));
      endMark = reader.getMark();
    } else {
      endMark = reader.getMark();
      int ff = 0;
      while (SjlExtCharConstants.NULL_OR_LINEBR.hasNo(reader.peek(ff))) {
        ff++;
      }
      if (ff > 0) {
        reader.forward(ff);
    ...
    SjlExtCommentToken commentToken = scanDirectiveIgnoredLine(startMark);
    SjlExtDirectiveToken token = new SjlExtDirectiveToken(name, value, startMark, endMark);
   ...
   */
  private String scanDirectiveName(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    int length = 0;
    ...
    while (SjlExtCharConstants.ALPHA.has(c)) {
      length++;
    ...
    if (length == 0) {
      ...
      throw new SjlExtScannerException(DIRECTIVE_PREFIX, startMark,
          EXPECTED_ALPHA_ERROR_PREFIX + s + "(" + c
              + ")", reader.getMark());
    }
    String value = reader.prefixForward(length);
    c = reader.peek();
    if (SjlExtCharConstants.NULL_BL_LINEBR.hasNo(c)) {
      final String s = String.valueOf(Character.toChars(c));
      throw new SjlExtScannerException(DIRECTIVE_PREFIX, startMark,
          EXPECTED_ALPHA_ERROR_PREFIX + s + "(" + c
              + ")", reader.getMark());
  ...
  private List<Integer> scanYamlDirectiveValue(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    while (reader.peek() == ' ') {
      reader.forward();
    }
   ...
   */
  private Integer scanYamlDirectiveNumber(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    int c = reader.peek();
   ...
   * </p>
   */
  private List<String> scanTagDirectiveValue(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    while (reader.peek() == ' ') {
      reader.forward();
    }
   ...
   */
  private String scanTagDirectiveHandle(Optional<SjlExtMark> startMark) {
    // See the specification for details.
   ...
   */
  private String scanTagDirectivePrefix(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    String value = scanTagUri("directive", SjlExtCharConstants.URI_CHARS_FOR_TAG_PREFIX, startMark);
    int c = reader.peek();
    if (SjlExtCharConstants.NULL_BL_LINEBR.hasNo(c)) {
      final String s = String.valueOf(Character.toChars(c));
      throw new SjlExtScannerException(DIRECTIVE_PREFIX, startMark,
  ...
  private SjlExtCommentToken scanDirectiveIgnoredLine(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    while (reader.peek() == ' ') {
      reader.forward();
    }
    SjlExtCommentToken commentToken = null;
    if (reader.peek() == '#') {
      SjlExtCommentToken comment = scanComment(SjlExtCommentType.IN_LINE);
      if (settings.getParseComments()) {
   ...
   * </pre>
   */
  private SjlExtToken scanAnchor(boolean isAnchor) {
    Optional<SjlExtMark> startMark = reader.getMark();
    ...
    while (SjlExtCharConstants.NULL_BL_T_LINEBR.hasNo(c, ",[]{}/.*&")) {
      length++;
      c = reader.peek(length);
    }
    if (length == 0) {
      final String s = String.valueOf(Character.toChars(c));
      throw new SjlExtScannerException("while scanning an " + name, startMark,
          "unexpected character found " + s + "(" + c + ")", reader.getMark());
    }
    String value = reader.prefixForward(length);
    c = reader.peek();
    if (SjlExtCharConstants.NULL_BL_T_LINEBR.hasNo(c, "?:,]}%@`")) {
      final String s = String.valueOf(Character.toChars(c));
      throw new SjlExtScannerException("while scanning an " + name, startMark,
          "unexpected character found " + s + "(" + c + ")", reader.getMark());
    }
    ...
    SjlExtToken tok;
    if (isAnchor) {
      tok = new SjlExtAnchorToken(new SjlExtAnchor(value), startMark, endMark);
    } else {
      tok = new SjlExtAliasToken(new SjlExtAnchor(value), startMark, endMark);
    }
    return tok;
  }
  ...
  private SjlExtToken scanTag() {
    ...
    if (c == '<') {
      ...
      // a URI).
      reader.forward(2);
      suffix = scanTagUri("tag", SjlExtCharConstants.URI_CHARS_FOR_TAG_PREFIX, startMark);
      c = reader.peek();
      if (c != '>') {
        // If there are any characters between the end of the tag-suffix
        // URI and the closing &gt;, then an error has occurred.
        final String s = String.valueOf(Character.toChars(c));
        throw new SjlExtScannerException("while scanning a tag", startMark,
            "expected '>', but found '" + s + "' (" + c
                + ")", reader.getMark());
      }
      reader.forward();
    } else if (SjlExtCharConstants.NULL_BL_T_LINEBR.has(c)) {
      ...
      while (SjlExtCharConstants.NULL_BL_LINEBR.hasNo(c)) {
        if (c == '!') {
          useHandle = true;
          break;
        }
      ...
      suffix = scanTagUri("tag", SjlExtCharConstants.URI_CHARS_FOR_TAG_SUFFIX, startMark);
    }
    c = reader.peek();
    ...
    SjlExtTagTuple value = new SjlExtTagTuple(Optional.ofNullable(handle), suffix);
    Optional<SjlExtMark> endMark = reader.getMark();
    return new SjlExtTagToken(value, startMark, endMark);
  }
   ...
   */
  private List<SjlExtToken> scanBlockScalar(SjlExtScalarStyle style) {
    // See the specification for details.
    ...
    SjlExtCommentToken commentToken = scanBlockScalarIgnoredLine(startMark);
    ...
    if (chomping.increment.isPresent()) {
      ...
      endMark = (Optional<SjlExtMark>) brme[1];
    } else {
      ...
      endMark = (Optional<SjlExtMark>) brme[2];
    ...
    if (this.reader.getColumn() < blockIndent && this.indent != reader.getColumn()) {
      ...
      throw new SjlExtScannerException("while scanning a block scalar", startMark,
    ...
    // We are done.
    SjlExtScalarToken scalarToken = new SjlExtScalarToken(stringBuilder.toString(), false, style, startMark,
        endMark);
   ...
   */
  private Chomping scanBlockScalarIndicators(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    ...
    if (c == '-' || c == '+') {
      ...
      if (Character.isDigit(c)) {
        ...
        if (incr == 0) {
          throw new SjlExtScannerException(SCANNING_SCALAR, startMark,
   ...
   */
  private SjlExtCommentToken scanBlockScalarIgnoredLine(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    ...
    SjlExtCommentToken commentToken = null;
    if (reader.peek() == '#') {
      commentToken = scanComment(SjlExtCommentType.IN_LINE);
    }
  ...
  private Object[] scanBlockScalarIndentation() {
    ...
    while (SjlExtCharConstants.LINEBR.has(reader.peek(), " \r")) {
      if (reader.peek() != ' ') {
   ...
   * </pre>
   */
  private SjlExtToken scanFlowScalar(final SjlExtScalarStyle style) {
    // The style will be either single- or double-quoted; we determine this
    // by the first character in the entry (supplied)
    final boolean doubleValue = style == SjlExtScalarStyle.DOUBLE_QUOTED;
    StringBuilder chunks = new StringBuilder();
    Optional<SjlExtMark> startMark = reader.getMark();
    ...
    reader.forward();
    Optional<SjlExtMark> endMark = reader.getMark();
    return new SjlExtScalarToken(chunks.toString(), false, style, startMark, endMark);
  }

  /**
   * Scan some number of flow-scalar non-space characters.
   */
  private String scanFlowScalarNonSpaces(boolean doubleQuoted, Optional<SjlExtMark> startMark) {
    // See the specification for details.
    StringBuilder chunks = new StringBuilder();
    while (true) {
      ...
      int length = 0;
      while (SjlExtCharConstants.NULL_BL_T_LINEBR.hasNo(reader.peek(length), "'\"\\")) {
        length++;
      }
      if (length != 0) {
      ...
      if (!doubleQuoted && c == '\'' && reader.peek(1) == '\'') {
        ...
        if (!Character.isSupplementaryCodePoint(c) && ESCAPE_REPLACEMENTS.containsKey(
          ...
          if (NOT_HEXA.matcher(hex).find()) {
            throw new SjlExtScannerException("while scanning a double-quoted scalar",
          ...
          final String s = String.valueOf(Character.toChars(c));
          throw new SjlExtScannerException("while scanning a double-quoted scalar", startMark,
  ...
  private String scanFlowScalarSpaces(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    StringBuilder chunks = new StringBuilder();
    int length = 0;
    ...
    if (c == 0) {
      ...
      throw new SjlExtScannerException("while scanning a quoted scalar", startMark,
  ...
  private String scanFlowScalarBreaks(Optional<SjlExtMark> startMark) {
    // See the specification for details.
    StringBuilder chunks = new StringBuilder();
    while (true) {
      ...
      if (("---".equals(prefix) || "...".equals(prefix))
          && SjlExtCharConstants.NULL_BL_T_LINEBR.has(reader.peek(3))) {
        throw new SjlExtScannerException("while scanning a quoted scalar", startMark,
  ...
  private SjlExtToken scanPlain() {
    ...
    while (true) {
      ...
      while (true) {
        c = reader.peek(length);
        if (SjlExtCharConstants.NULL_BL_T_LINEBR.has(c)
            || (c == ':' && SjlExtCharConstants.NULL_BL_T_LINEBR.has(reader.peek(length + 1),
            isFlowContext() ? ",[]{}" : ""))
    ...
    return new SjlExtScalarToken(chunks.toString(), true, startMark, endMark);
  }
  ...
  private boolean atEndOfPlain() {
    ...
    {
      int c;
      while ((c = reader.peek(wsLength)) != 0 && SjlExtCharConstants.NULL_BL_T_LINEBR.has(c)) {
        wsLength++;
        if (!SjlExtCharConstants.LINEBR.has(c) && (c != '\r' || reader.peek(wsLength + 1) != '\n')
            && c != 0xFEFF) {
          wsColumn++;
        } else {
          wsColumn = 0;
    ...
    if (isBlockContext()) {
      int c;
      for (int extra = 1;
          (c = reader.peek(wsLength + extra)) != 0 && !SjlExtCharConstants.NULL_BL_T_LINEBR.has(c);
          extra++) {
        if (c == ':' && SjlExtCharConstants.NULL_BL_T_LINEBR.has(reader.peek(wsLength + extra + 1))) {
          return true;
        }
      }
    }
   ...
   * </pre>
   */
  private String scanTagHandle(String name, Optional<SjlExtMark> startMark) {
    int c = reader.peek();
    if (c != '!') {
      final String s = String.valueOf(Character.toChars(c));
      throw new SjlExtScannerException(SCANNING_PREFIX + name, startMark,
          "expected '!', but found " + s + "(" + (c) + ")", reader.getMark());
    }
   ...
   * URI specification.
   */
  private String scanTagUri(String name, SjlExtCharConstants range, Optional<SjlExtMark> startMark) {
    // See the specification for details.
   ...
   */
  private String scanUriEscapes(String name, Optional<SjlExtMark> startMark) {
    ...
    // separate character.
    Optional<SjlExtMark> beginningMark = reader.getMark();
    ...
    buff.flip();
    try {
      return SjlExtUriEncoder.decode(buff);
   ...
   */
  private List<SjlExtToken> makeTokenList(SjlExtToken... tokens) {
    List<SjlExtToken> tokenList = new ArrayList<>();
    for (int ix = 0; ix < tokens.length; ix++) {
      if (tokens[ix] == null) {
        continue;
      }
      if (!settings.getParseComments() && (tokens[ix] instanceof SjlExtCommentToken)) {
        continue;
      }
